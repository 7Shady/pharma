(function(window) {
	...
	Calculator.prototype.calc = function(key, val) {
		...
		key = key.replace('Ã—', '*').replace('Ã·', '/').replace('â€“', '-');
		if (key !== '=') {
			this.buff[1] = key;
		...
		if (key === '=' && !this.stack[0] && this.curr && this.buff[1]) { // repeating '='
			return (this.buff[1] === 'yx' ? Math.pow(val, this.buff[0]) : this.buff[1] === 'xâˆšy' ?
		...
		if (rank[key] <= rank[this.stack[this.num - 1][1]]) {
			this.stack[this.num - 1] = [
				...
				this.stack[this.num - 1][1] === 'xâˆšy' ? Math.pow(this.stack[this.num - 1][0], 1 / val) :
	...
	document.addEventListener('keypress', function(e) {
		var key = e.which,
			...
			keyMatch = (',|.|-|â€“|/|Ã·|*|Ã—|#|+/â€“|x|x!|E|EE|e|ex| |2nd|r|xâˆšy|R|âˆš|p|Ï€|^|yx|\'|yx|"|yx|m|mr|v|mc|b|m+|n|m-|' +
				's|sin|c|cos|t|tan|S|sin-1|C|cos-1|T|tan-1|d|Deg|Â°|Deg|l|ln|L|log|\\|1/x|X|2x').split('|'),
			keyMatchHold = ('sin|sinh|cos|cosh|tan|tanh|m-|Rand|Deg|Rand|sin-1|sinh-1|cos-1|cosh-1|tan-1|tanh-1|' +
				'1|1/x|2|x2|3|x3|xâˆšy|âˆš|ln|log2|ex|2x').split('|');
	...
	function doKey(text, alt) {
		...
		} else if (text.match(/^[+|â€“|Ã·|Ã—|yx|xâˆšy|E]+$/) && text !== 'âˆš') {
			freezeKey(key);
		} else if (text.match(/^[\d|\.|Ï€]$/)) {
			freezeKey(key, true);
			keyBoard['AC'].children[0].firstChild.data = 'C';
		} else if (text === 'C') {
	...
	function evalKey(key) {
		var dispVal = resBuffer.replace(/\s/g, '').replace(/Error|âˆž|-âˆž/, '0') + '',
			_PI = Math.PI,
			lastKey;
		...
		if (key.match(/^[\d|\.]$/) || key === '+/â€“') {
			if (calculator[brackets].curr && key !== '+/â€“' || (key === '+/â€“' &&
					lastKey && lastKey.match(/^[+|â€“|Ã·|Ã—|yx|xâˆšy|E|^C]+$/))) {
				dispVal = '0';
			...
			if ((Math.abs(+(dispVal + key)) > (bigger ? 1e15 : 1e9) ||
					...
					(dispVal.match(/\.|\e\+/) && key === '.')) && key !== '+/â€“') {
				buffStr.pop();
				return;
			} else if (key === '+/â€“') {
				render(!(dispVal.replace(/e[\+|\-]/, '')).match('-') ?
					'-' + dispVal : dispVal.replace(/^-/, ''), '+/â€“');
			} else {
			...
			hold.textContent = '';
		} else if (key.match(/^[+|â€“|Ã·|Ã—|-|\/|*|yx|xâˆšy|%|E]+$/) && key !== 'âˆš') {
